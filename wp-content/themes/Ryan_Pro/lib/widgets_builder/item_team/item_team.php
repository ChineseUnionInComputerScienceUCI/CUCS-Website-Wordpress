...
/* ----------------------------------------------------------------------------------
	Custom Post Item - Team
---------------------------------------------------------------------------------- */

class thinkup_builder_itemteam extends WP_Widget {

	/* Register widget description. */
	public function __construct() {
		$widget_ops = array('classname' => 'thinkup_builder_itemteam', 'description' => 'Add a team member to your content.' );
		parent::__construct('thinkup_builder_itemteam', 'Team (Item)', $widget_ops);
	}
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('image_round') . '">Round image?</label>&nbsp;<input id="' . $this->get_field_id('image_round') . '" name="' . $this->get_field_name('image_round') . '" type="checkbox" '; ?><?php if($image_round == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 65px;" /></p>';

		echo '<p><label for="' . $this->get_field_id('show_link') . '" style="display: inline-block;width: 150px;">Show Link:</label>
			<select name="' . $this->get_field_name('show_link') . '" id="' . $this->get_field_id('show_link') . '" class="widget-thinkup_builder_itemteam" style="display: inline-block;width: 200px;margin: 0;">
			<option '; ?><?php if($show_link == "on") { echo "selected"; } ?><?php echo ' value="on">On</option>
			<option '; ?><?php if($show_link == "off") { echo "selected"; } ?><?php echo ' value="off">Off</option>
			</select>
		</p>';

		echo '<p><label for="' . $this->get_field_id('show_name') . '">Show Member Name?</label>&nbsp;<input id="' . $this->get_field_id('show_name') . '" name="' . $this->get_field_name('show_name') . '" type="checkbox" '; ?><?php if($show_name == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 18px;" /></p>';

		echo '<p><label for="' . $this->get_field_id('show_position') . '">Show Position?</label>&nbsp;<input id="' . $this->get_field_id('show_position') . '" name="' . $this->get_field_name('show_position') . '" type="checkbox" '; ?><?php if($show_position == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 59px;" /></p>';

		echo '<p><label for="' . $this->get_field_id('show_excerpt') . '">Show Excerpt?</label>&nbsp;<input id="' . $this->get_field_id('show_excerpt') . '" class="widget-thinkup_builder_item" name="' . $this->get_field_name('show_excerpt') . '" type="checkbox" '; ?><?php if($show_excerpt == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 63px;" /></p>';

		echo '<p><label for="' . $this->get_field_id('show_social') . '">Show Social Icons?</label>&nbsp;<input id="' . $this->get_field_id('show_social') . '" name="' . $this->get_field_name('show_social') . '" type="checkbox" '; ?><?php if($show_social == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 39px;" /></p>';

		echo '<p><label for="' . $this->get_field_id('center') . '">Center Align?</label>&nbsp;<input id="' . $this->get_field_id('center') . '" name="' . $this->get_field_name('center') . '" type="checkbox" '; ?><?php if($center == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 72px;" /></p>';
	...
	function update($new_instance, $old_instance) {
		...
		$instance['image_round']    = $new_instance['image_round'];
	...
	function widget($args, $instance) {
		...
		echo do_shortcode( '[item_team id="' . $post_id . '" image_round=' . $image_round . ' size="' . $size . '" show_link="' . $show_link . '" show_name="' . $show_name . '" show_position="' . $show_position . '" show_excerpt="' . $show_excerpt . '" show_social="' . $show_social . '" center="' . $center . '" style="' . $style . '" ]' );
...
add_action( 'widgets_init', create_function('', 'return register_widget("thinkup_builder_itemteam");') );
