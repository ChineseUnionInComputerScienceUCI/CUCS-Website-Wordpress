...
if ( ! class_exists( 'thinkup_widget_categories' ) ) {
	class thinkup_widget_categories extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Categories', 'ryan' ) : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
					...
					echo '<ul>';
						foreach( $terms as $term ) {
						  echo '<li class="cat-item cat-item-24">',
								'<a href="' . get_category_link( $term->term_id ) . '" title="' . sprintf( __( 'View all posts in %s', 'ryan' ), $term->name ) . '" ' . '><span class="cat-name">' . $term->name . '</span>';
									if ($instance['postswitch'] == 'on') {
										echo '<span class="cat-count">(' . $term->count.')</span>';
									}
						  echo 	'</a></li> ';
						}
					echo '</ul>';
