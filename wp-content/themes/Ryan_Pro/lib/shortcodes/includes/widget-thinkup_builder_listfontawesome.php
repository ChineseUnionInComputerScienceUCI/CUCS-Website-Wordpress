...
class thinkup_builder_listfontawesome extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'thinkup_builder_listfontawesome-id',
			__( 'List (Font Awesome)', 'thinkupshortcodes' ),
			array(
				'description'	=>	__( 'Add a custom list to your content.', 'thinkupshortcodes' )
			)
		,array( 'width' => 495)
	...
	public function widget( $args, $instance ) {
		...
		$element = Thinkupshortcodes::get_instance();
		echo $element->render_element($instance, '', 'thinkup_builder_listfontawesome');		
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/thinkup_builder_listfontawesome-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-thinkup_builder_listfontawesome.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-thinkup_builder_listfontawesome.php';		
		}else{
			return;
		}

		echo "<input type=\"hidden\" name=\"thinkupshortcodes-widget\">\r\n";
		$groups = array();
		$setsize = 'full';
		...
		foreach ($configfiles as $key=>$fieldfile) {
			...
			echo "<div class=\"thinkupshortcodes-widget-config-nav\">\r\n";
			echo "	<ul>\r\n";
			...
			echo "<div class=\"thinkupshortcodes-widget-config-content " . $setsize . "\">\r\n";
			foreach ($groups as $key=>$group) {
				echo "<div id=\"row".self::get_field_id('__row'.$group['id'])."\" class=\"thinkupshortcodes-groupbox group\" " . ( !empty($instance['__cur_tab__']) ? ($instance['__cur_tab__'] == $key ? "" : "style=\"display:none;\"") : ($key === 0 ? "" : "style=\"display:none;\"" )) . ">\r\n";
	...
	public function group($group, $instance){
		...
		for( $i=0; $i<$depth;$i++ ){
			...
			echo "<div class=\"form-table rowGroup groupitems\" id=\"groupitems\" ref=\"items\">\r\n";
				foreach($group['fields'] as $field=>$settings){
					...
					echo '<div class="thinkupshortcodes-field-row"><label class="thinkupshortcodes_widget_label" for="'.$id.'">'.$label.'</label>';
		...
		if(!empty($group['multiple'])){
			echo "<div><button class=\"button thinkupshortcodes-add-group-row\" type=\"button\" data-field=\"".self::get_field_id('ref')."\" data-rowtemplate=\"group-".$group['id']."-tmpl\">".__('Add Another','thinkupshortcodes')."</button></div>\r\n";
		}
	...
	public function widget_textdomain() {
		...
		load_plugin_textdomain( 'thinkupshortcodes', false, self::get_path( dirname( __FILE__ ) ) . '/lang/' );
		...
		foreach ($configfiles as $key=>$fieldfile) {
			...
			if(!empty($group['scripts'])){
				foreach($group['scripts'] as $script){
					wp_enqueue_script( 'thinkupshortcodes-'.strtok($script, '.'), self::get_url( 'assets/js/'.$script , dirname(__FILE__) ) , array('jquery') );					
				}
			}
			if(!empty($group['styles'])){
				foreach($group['styles'] as $style){
					wp_enqueue_style( 'thinkupshortcodes-'.strtok($style, '.'), self::get_url( 'assets/css/'.$style , dirname(__FILE__) ) );
				}
			}
		}

		wp_enqueue_style( 'thinkupshortcodes-panel-styles', self::get_url( 'assets/css/panel.css', dirname(__FILE__) ) );
		wp_enqueue_script( 'thinkupshortcodes-panel-script', self::get_url( 'assets/js/panel.js', dirname(__FILE__) ), array( 'jquery' ) );
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'thinkup_builder_listfontawesome', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("thinkup_builder_listfontawesome");' ) );
