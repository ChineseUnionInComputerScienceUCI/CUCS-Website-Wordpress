...
class thinkup_builder_toggle extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'thinkup_builder_toggle-id',
			__( 'Toggle / Accordion', 'thinkupshortcodes' ),
			array(
				'description'	=>	__( 'Add a toggle / accordion to your content.', 'thinkupshortcodes' )
			)
		,array( 'width' => 495)
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'thinkup_builder_toggle');		
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/thinkup_builder_toggle-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-thinkup_builder_toggle.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-thinkup_builder_toggle.php';		
		}else{
			return;
		}
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'thinkup_builder_toggle', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("thinkup_builder_toggle");' ) );
