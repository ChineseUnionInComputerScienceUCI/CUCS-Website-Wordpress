...
class thinkup_builder_buttontheme extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'thinkup_builder_buttontheme-id',
			__( 'Button', 'thinkupshortcodes' ),
			array(
				'description'	=>	__( 'Add a button to your content.', 'thinkupshortcodes' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'thinkup_builder_buttontheme');		
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/thinkup_builder_buttontheme-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-thinkup_builder_buttontheme.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-thinkup_builder_buttontheme.php';		
		}else{
			return;
		}
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'thinkup_builder_buttontheme', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("thinkup_builder_buttontheme");' ) );
