...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo esc_attr( $table_id ); ?>" class="add-row-button button" href="#"><?php _e( 'Add Row', 'cmb' ); ?></a>
		</p>
	...
	// ========================================================================
	// ThinkUpThemes customization - START
	// ========================================================================

	// Custom field - Select Category
	public function category_select() {
		...
		// Set default option
		$options .= $this->option( 'Select a category:', 'Select a category:', $selected );
		
		// Begin loop to add all other options
		foreach ( $categories as $category ) {
			if( $category->term_id == $this->field->value ) {
				$selected = $saved_term == $this->field->value;
			} else {
				$selected = NULL;
			}
			$options .= $this->option( $category->name, $category->term_id, $selected );
		}

		return $this->select( array( 'options' => $options ) );
	}

	// Custom field - Select Excerpt Length
	public function excerpt_select() {

		$saved_term = $this->field->value;

		// Set default option
		$options .= $this->option( 'Default', 'default', $selected );
		
		// Begin loop to add all other options
		foreach ( range(1,150) as $k => $v ) {
			if( $v == $this->field->value ) {
				$selected = $saved_term == $this->field->value;
			} else {
				$selected = NULL;
			}
			$options .= $this->option( $v, $v, $selected );
		}

		return $this->select( array( 'options' => $options ) );
	}

	// Custom field - Select Pagination Style
	...
	public function sidebar_select() {
		...
		// Set default option
		$options .= $this->option( 'Select a sidebar:', 'Select a sidebar:', $selected );
		
		// Begin loop to add all other options
		foreach ( $wp_registered_sidebars as $sidebar ) {
			if( $sidebar['name'] == $this->field->value ) {
				$selected = $saved_term == $this->field->value;
			} else {
				$selected = NULL;
			}

			// Only display non-footer sidebars 
			if ( strpos( $sidebar['id'], 'footer' ) !== false ) {
			} else {
				$options .= $this->option( $sidebar['name'], $sidebar['name'], $selected );
			}
		}
	...
	// Custom field - Select Team Member
	public function team_select() {
		...
		// Set default option
		$options .= $this->option( 'Select a Team Member:', '', $selected );

		// Begin loop to add all other options
		foreach ( $pages as $page ) {
			if( $page->ID == $this->field->value ) {
				$selected = $saved_term == $this->field->value;
			} else {
				$selected = NULL;
			}
			...
			// Only display non-footer sidebars 
			$options .= $this->option( $page->post_title, $page->ID, $selected );
		}

		return $this->select( array( 'options' => $options ) );
	}

	// Custom field - Select Page
	public function page_select() {
		...
		// Set default option
		$options .= $this->option( '(no parent)', '', $selected );
	...
	// ========================================================================
	// ThinkUpThemes customization - END
	// ========================================================================
