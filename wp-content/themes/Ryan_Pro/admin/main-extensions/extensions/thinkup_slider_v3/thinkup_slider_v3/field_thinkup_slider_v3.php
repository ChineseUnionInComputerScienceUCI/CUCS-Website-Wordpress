 ...
 * @subpackage  Field_thinkup_slider_v3
...
// Don't duplicate me!
if ( !class_exists ( 'ReduxFramework_thinkup_slider_v3' ) ) {

    /**
     * Main ReduxFramework_thinkup_slider_v3 class
     *
     * @since       1.0.0
     */
    class ReduxFramework_thinkup_slider_v3 {
...
//                    '***EXAMPLE-NEW-VARIABLE***' => true, // Copy this line to add a new field
            ...
            echo '<div class="redux-thinkup_slider_v3-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'redux-framework' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                $thinkup_slider_v3 = $this->value;

                foreach ( $thinkup_slider_v3 as $slide ) {
...
//                        '***EXAMPLE-NEW-VARIABLE***' => '', // Copy this line to add a new field
                    ...
                    echo '<div class="redux-thinkup_slider_v3-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-thinkup_slider_v3-header">' . $slide[ 'slide_title' ] . '</span></h3><div>';
...
//						$hide = ' hide';
                    ...
                    echo '<img class="redux-thinkup_slider_v3-image" id="image_image_id_' . $x . '" src="' . $slide[ 'slide_image_thumb' ] . '" alt="" target="_blank" rel="external" />';
                    echo '</a>';
                    echo '</div>';

                    echo '<div class="redux_thinkup_slider_v3_add_remove">';
                    ...
                    echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="redux-thinkup_slider_v3-list">';

					// Add field - slide_video
                    if ( $this->field[ 'show' ][ 'slide_video' ] ) {
                        $title_type = "text";
                    } else {
                        $title_type = "hidden";
                    }
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'slide_video' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'slide_video' ] ) : __ ( 'Video URL', 'redux-framework' );
                    echo '<li><input type="' . $title_type . '" id="' . $this->field[ 'id' ] . '-slide_video_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][slide_video]' . $this->field['name_suffix'] . '" value="' . esc_attr ( $slide[ 'slide_video' ] ) . '" placeholder="' . $placeholder . '" class="full-text" /></li>';
					...
					// Add field - slide_button
                    if ( $this->field[ 'show' ][ 'slide_button' ] ) {
						$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'slide_button' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'slide_button' ] ) : __ ( 'Button Text', 'redux-framework' );
						echo '<li><input type="' . $title_type . '" id="' . $this->field[ 'id' ] . '-slide_button_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][slide_button]' . $this->field['name_suffix'] . '" value="' . esc_attr ( $slide[ 'slide_button' ] ) . '" placeholder="' . $placeholder . '" class="full-text" /></li>';
                    }
					...
					// Add field - slide_class
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'slide_class' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'slide_class' ] ) : __ ( 'Style classes (comma separated)', 'redux-framework' );
                    if ( $this->field[ 'show' ][ 'slide_class' ] ) {
                        $url_type = "text";
                    } else {
                        $url_type = "hidden";
                    }
                    echo '<li><input type="' . $url_type . '" id="' . $this->field[ 'id' ] . '-slide_class_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][slide_class]' . $this->field['name_suffix'] .'" value="' . esc_attr ( $slide[ 'slide_class' ] ) . '" class="full-text" placeholder="' . $placeholder . '" /></li>';
...
/*                    if ( $this->field[ 'show' ][ '***EXAMPLE-NEW-VARIABLE***' ] ) {
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-thinkup_slider_v3-remove">' . __ ( 'Delete', 'redux-framework' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="redux-thinkup_slider_v3-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-thinkup_slider_v3-header">' . esc_attr ( sprintf ( __ ( 'New %s', 'redux-framework' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';
...
//				$hide = ' hide';
                ...
                echo '<img class="redux-thinkup_slider_v3-image" id="image_image_id_' . $x . '" src="" alt="" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
				...
				// Add field - slide_video (Empty Slider)
                if ( $this->field[ 'show' ][ 'slide_video' ] ) {
                    $title_type = "text";
                } else {
                    $title_type = "hidden";
                ...
                $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'slide_video' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'slide_video' ] ) : __ ( 'Video URL', 'redux-framework' );
                echo '<li><input type="' . $title_type . '" id="' . $this->field[ 'id' ] . '-slide_video_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][slide_video]' . $this->field['name_suffix'] .'" value="" placeholder="' . $placeholder . '" class="full-text" /></li>';
				...
				// Add field - slide_button (Empty Slider)
				if ( $this->field[ 'show' ][ 'slide_button' ] ) {
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'slide_button' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'slide_button' ] ) : __ ( 'Button Text', 'redux-framework' );
					echo '<li><input type="' . $title_type . '" id="' . $this->field[ 'id' ] . '-slide_button_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][slide_button]' . $this->field['name_suffix'] .'" value="" placeholder="' . $placeholder . '" class="full-text" /></li>';
                }
				...
				// Add field - slide_class (Empty Slider)
                $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'slide_class' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'slide_class' ] ) : __ ( 'Slider classes (comma separated)', 'redux-framework' );
                if ( $this->field[ 'show' ][ 'slide_class' ] ) {
                    $url_type = "text";
                } else {
                    $url_type = "hidden";
                }
                echo '<li><input type="' . $url_type . '" id="' . $this->field[ 'id' ] . '-slide_class_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][slide_class]' . $this->field['name_suffix'] .'" value="" class="full-text" placeholder="' . $placeholder . '" /></li>';
...
/*				if ( $this->field[ 'show' ][ '***EXAMPLE-NEW-VARIABLE***' ] ) {
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-thinkup_slider_v3-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'redux-framework' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue () {
            ...
            wp_enqueue_style (
                'thinkup-field-thinkup_slider_v3-css', 
                $this->_extension_url . 'field_thinkup_slider_v3.css', 
                array(),
                time (), 
                'all'
            );
            ...
            wp_enqueue_script (
                'thinkup-field-thinkup_slider_v3-js',
				$this->_extension_url . 'field_thinkup_slider_v3.js',
